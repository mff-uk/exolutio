...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron">
  <!-- Generated by eXolutio on 9.4.2012 20:56 from tournaments/MatchSchedule. -->
  ...
  <sch:pattern id="Tournament">
    <sch:rule context="/tournament">
      ...
      <sch:assert test="oclX:forAll(oclX:collect(matches/day, function($d) { $d/match }), function($m) { $m/start ge $self/start and $m/end le $self/end })">All matches in a tournament occur within the tournament's time frame</sch:assert>
      <!--self.matches.day->collect(d : Day | d.match)->exists(m : Match | m.start.trunc() = self.start.trunc())-->
      <sch:assert test="oclX:exists(oclX:collect(matches/day, function($d) { $d/match }), function($m) { oclDate:trunc($m/start) eq oclDate:trunc($self/start) })">Each tournament conducts at least one match on the first day of the tournament</sch:assert>
      <!--self.matches.day->forAll(d1 : Day, d2 : Day | d1 <> d2 implies d1.date <> d2.date)-->
      <sch:assert test="oclX:forAllN(matches/day, function($d1, $d2) { if (not($d1 is $d2)) then $d1/date ne $d2/date else true() })">Days must be unique</sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <sch:pattern id="Day">
    <sch:rule context="/tournament/matches/day">
      ...
      <sch:assert test="oclX:forAll(match, function($m) { oclDate:getDate($m/start) eq $self/date })">Each day shows only matches taking place that day</sch:assert>
    </sch:rule>
  </sch:pattern>
