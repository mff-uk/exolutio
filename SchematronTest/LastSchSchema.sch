...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron">
  <!-- Generated by eXolutio on 24.2.2012 23:34 from tournaments/MatchSchedule. -->
  <sch:pattern id="collections">
    <!--Below follow constraints from OCL script 'collections'. -->
    <sch:rule context="/tournament">
      <context-variable name="t" xmlns="http://eXolutio.com/oclX" />
      <!--t.matches.day->Collect(d : Day | d.match)->forAll(m : Match | m.start.after(t.start) and m.end.before(t.end))-->
      <sch:assert test="oclX:forAll(oclX:collect(matches/day, 'd', '$d/match', $variables), 'm', 'oclDate:after($m/start, $t/start) and oclDate:before($m/end, $t/end)', $variables)" />
    </sch:rule>
    <sch:rule context="/tournament">
