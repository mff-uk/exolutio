...
<sch:schema 
  ...
  <sch:pattern>
    ...
    <sch:rule context="/tournament">
      <sch:assert test="
        oclX:forAll(oclX:collect(matches/day, 'it', '$it/match', $variables),'m','oclDate:after($m/start, $self/start) and oclDate:before($m/end, $self/end)',oclX:vars(.)) ">.
      </sch:assert>      
    </sch:rule>
    ...
    <sch:rule context="/tournament">
      <sch:assert test="oclX:exists(matches/day/match, 'm', 'm/start/ eq start')">
      </sch:assert>      
    </sch:rule>
    ...
    <sch:rule context="/tournament/matches/day/match">
      <sch:assert test="
        oclX:forAll(match-players/player,'p', 'ocl:exists(p/../../../../../participating-players/player, &apos;px&apos;, &apos;px/name = p/name&apos;)')">
      </sch:assert>      
    </sch:rule>      
