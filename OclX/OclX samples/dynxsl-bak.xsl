...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/departments">
        ...
        <xsl:variable name="variables1" as="element()*">
    ...
    <xsl:function name="oclX:collect">
    ...
    <xsl:function name="oclX:iterate">
        <xsl:param name="collection" />
        <xsl:param name="accumulatorInitExpresson" />        
        <xsl:param name="expression" />$accumulator, $collection[$iteration]
        <xsl:param name="variables" as="element()*" />
        
        <xsl:variable name="accumulatorInitialValue">
            <xsl:copy-of select="saxon:evaluate($accumulatorInitExpresson)"></xsl:copy-of>
        </xsl:variable>            
        
        <xsl:value-of select="oclX:iterate-rec($collection, 
        ...
        <xsl:choose>            
            ...
            <xsl:otherwise>
                <xsl:variable name="newAccumulator">
                    <xsl:value-of select="saxon:evaluate($expression, $variables[1]/*[1], $accumulator, $collection[$iteration])" />                     
                </xsl:variable>
                <xsl:value-of select="oclX:iterate-rec($collection, $newAccumulator, $expression, $iteration +1, $totalIterations, $variables)"/>                                
            </xsl:otherwise>            
        </xsl:choose>   
    </xsl:function>
