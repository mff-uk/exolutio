...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron">
  <!-- Generated by eXolutio on 27.3.2012 22:55 from tournaments/MatchSchedule. -->
  ...
  <sch:pattern id="Tournament">
    <sch:rule context="/tournament">
      ...
      <sch:assert test="oclX:forAll(oclX:collect(matches/day, 'd', '$d/match', $variables), 'm', 'oclDate:after($m/start, start) and oclDate:before($m/end, end)', $variables)" />
    </sch:rule>
    <sch:rule context="/tournament">
      <sch:let name="self" value="." />
      <!--self.matches.day->collect(d : Day | d.match)->exists(m : Match | m.start = self.start)-->
      <sch:assert test="oclX:exists(oclX:collect(matches/day, 'd', '$d/match', $variables), 'm', '$m/start eq start', $variables)" />
    </sch:rule>
  </sch:pattern>
