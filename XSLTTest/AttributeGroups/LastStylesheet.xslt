...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
  <!-- Template generated by eXolutio on 30.7.2011 20:10 
       from D:\Programování\EvoXSVN\XSLTTest\AttributeGroups\attributeGroups.eXo. -->
  ...
  <xsl:template name="TOP-Purchase-ATT">
    ...
    <xsl:for-each-group select="$ci[name() = 'city'] | $ci[name() = 'zip'] | $ci[name() = 'street'] | $ci[name() = 'customer-no'] | $ci[name() = 'email']" group-starting-with="customer-no">
  ...
  <xsl:template name="TOP-Purchase-Item-ATT">
    <xsl:param name="ci" as="item()*" />
    <xsl:for-each-group select="$ci[name() = 'subcode'] | $ci[name() = 'code'] | $ci[name() = 'weight'] | $ci[name() = 'title']" group-starting-with="code">
      <xsl:call-template name="TOP-Purchase-Item-Product-ATT">
        <xsl:with-param name="ci" select="current-group()" />
      </xsl:call-template>
    </xsl:for-each-group>
    <xsl:for-each-group select="$ci[name() = 'amount'] | $ci[name() = 'unit-price']" group-starting-with="amount">
  ...
  <xsl:template name="TOP-Purchase-CustomerInfo-ATT">
    <xsl:param name="ci" as="item()*" />
    <xsl:for-each-group select="$ci[name() = 'city'] | $ci[name() = 'zip'] | $ci[name() = 'street']" group-starting-with="zip">
      <xsl:call-template name="TOP-Purchase-CustomerInfo-Address-ATT">
        <xsl:with-param name="ci" select="current-group()" />
      </xsl:call-template>
    </xsl:for-each-group>
    <xsl:for-each-group select="$ci[name() = 'customer-no'] | $ci[name() = 'email']" group-starting-with="customer-no">
