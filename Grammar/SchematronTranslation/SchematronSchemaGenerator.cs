...
namespace Exolutio.Model.PSM.Grammar.SchematronTranslation
{
    public class SchematronSchemaGenerator
    {
        ...
        public XDocument GetSchematronSchema([NotNull]TranslationSettings translationSettings)
        {
            ...
            List<CompilerResult> successfullyCompiled = new List<CompilerResult>();
            foreach (OCLScript oclScript in PSMSchema.OCLScripts)
            {
                ...
                try
                {
                    ...
                    {
                        writeError = true;
                        compilerResult.Errors.CopyToLog(Log);
                    }
                    else
                    {
                        successfullyCompiled.Add(compilerResult);
                    }
                }
                catch (Exception)
                {
                ...
                if (writeError)
                {
                    XComment errorComment = new XComment(string.Format("OCL script '{0}' contains errors and thus can not be translated. ", oclScript.Name));
                    schSchema.Add(errorComment);
                }
            }

            GeneratePatterns(schSchema, successfullyCompiled, translationSettings);
        ...
        private void GeneratePatterns(XElement schSchema, List<CompilerResult> compilerResults, TranslationSettings translationSettings)
        {
            Dictionary<PSMClass, PatternInfo> patterns = new Dictionary<PSMClass, PatternInfo>();
            Dictionary<ClassifierConstraintBlock, PSMBridge> bridgeDict = new Dictionary<ClassifierConstraintBlock, PSMBridge>();
            List<ClassifierConstraintBlock> blocksFromAllScripts = new List<ClassifierConstraintBlock>();
            foreach (CompilerResult compilerResult in compilerResults)
            {
                foreach (ClassifierConstraintBlock constraintBlock in compilerResult.Constraints.ClassifierConstraintBlocks)
                {
                    bridgeDict.Add(constraintBlock, (PSMBridge) compilerResult.Bridge);
                    blocksFromAllScripts.Add(constraintBlock);
                }
            }

            {
                IEnumerable<IGrouping<PSMClass, ClassifierConstraintBlock>> grouped = blocksFromAllScripts.GroupBy(GetContextTag);
                IEnumerable<PSMClass> keys = grouped.GetKeys();

                foreach (IGrouping<PSMClass, ClassifierConstraintBlock> group in grouped)
                {
                    ...
                    foreach (ClassifierConstraintBlock constraint in group)
                    {
                        ...
                        TranslateInvariantsToXPath(constraint, ruleElement, bridgeDict[constraint], translationSettings);
                    }
        ...
        private PSMClass GetContextTag(ClassifierConstraintBlock cc)
        {
